...
void PublishLaserScanFan(rclcpp::Publisher<sensor_msgs::msg::LaserScan>::SharedPtr laser_pub, RawData *fan, std::string &frame_id, double min_dist, double max_dist, uint8_t inverted, uint8_t reversed)
{
	...
	// 修改作者: Kyhralus
	// 添加打印语句
    RCLCPP_INFO(rclcpp::get_logger("bluesea_node"), "LaserScan - Timestamp: %d.%09d", msg.header.stamp.sec, msg.header.stamp.nanosec);
    RCLCPP_INFO(rclcpp::get_logger("bluesea_node"), "LaserScan - Angle Min: %f, Angle Max: %f", msg.angle_min, msg.angle_max);
    for (size_t i = 0; i < msg.ranges.size(); ++i) {
        RCLCPP_INFO(rclcpp::get_logger("bluesea_node"), "LaserScan - Range %zu: %f", i, msg.ranges[i]);
    }
	// =====================   自己修改部分   ================
