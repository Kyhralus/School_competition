...
class VisionTrackingNode(Node):
    ...
    def image_callback(self, msg: Image) -> None:
        ...
        try:
            ...
            # laser_point, _ = self.detect_red_laser(frame, target_point)
            
            # 检测靶心
            circle_frame, target_point = self.detect_deepest_inner_circle(frame)
            
            # 检测激光点
            laser_point, laser_frame =  self.detect_red_laser(frame, target_point)
    ...
    def detect_deepest_inner_circle(self, frame: np.ndarray) -> tuple[np.ndarray, Optional[tuple[int, int]]]:
        """检测层级最多的轮廓结构中最里层的圆心"""
        # crop_frame = cv2.crop(frame, (0, 0, 1280, 720))
    ...
    def detect_red_laser(
            self,
            frame: np.ndarray, 
            target_point: Optional[tuple[int, int]],  # 修正类型注解
            min_area: int = 50,
            max_area: int = 1500,
            erode_iter: int = 1,
            dilate_iter: int = 2
        ...
        """优先选择靠近target_point的激光点（若存在），否则选择图像中间区域的点，兼顾面积和距离"""
        result_frame = frame.copy()
        h, w = frame.shape[:2]  # 获取图像高度和宽度
        reference_point = target_point if target_point is not None else (w // 2, h // 2)  # 基准点：优先用target_point，否则用图像中心
        ...
        # 标记基准点（区分是target_point还是图像中心）
        if target_point is not None:
            # 基准点是target_point（用红色标记）
            cv2.circle(result_frame, reference_point, 3, (0, 0, 255), -1)
            cv2.putText(result_frame, "Target", (reference_point[0]+10, reference_point[1]-10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        else:
            # 基准点是图像中心（用蓝色标记）
            cv2.circle(result_frame, reference_point, 3, (255, 0, 0), -1)
            cv2.putText(result_frame, "Center", (reference_point[0]+10, reference_point[1]-10),
